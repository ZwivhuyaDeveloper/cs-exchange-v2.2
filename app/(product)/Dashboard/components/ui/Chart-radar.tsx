...
interface VolumeData {
  ...
  volume24h: number;
  marketShare: number;
  actualMarketShare: number; // Store the actual percentage for tooltip
}
...
// Fetch function for 24h volume data
const fetchVolumeData = async (tokenSymbol: string): Promise<VolumeData[]> => {
  ...
  const volumeData = Array.from(exchangeMap.values())
    ...
    .slice(0, 6) // Top 6 exchanges for radar chart
    .map(ex => {
      const marketShare = totalVolume > 0 ? (ex.volume_24h / totalVolume) * 100 : 0;
      // Scale market share to make radar chart more pronounced (multiply by 3 for better visibility)
...
export function ChartRadarMultiple({ tokenSymbol }: VolumeRadarProps) {
  const {
    data: volumeData,
    isLoading,
    error,
    ...
    queryKey: ['volumeRadar', tokenSymbol.toLowerCase()],
    queryFn: () => fetchVolumeData(tokenSymbol),
