...
import BannerResearch from "./banner-research";


interface ResearchSectionProps {
  data: simpleResearchCard[];
}
...
export default function ResearchSection({ data }: ResearchSectionProps) {
  ...
  const paginationData = useMemo(() => {
    ...
    return shouldSkipLatest && latestReport
      ? filteredData.filter(post => post !== latestReport)
      : filteredData;
  }, [filteredData, selectedCategory, searchQuery, latestReport]);
  ...
  const showBanner = !selectedCategory && !searchQuery && latestReport;
  ...
  return (
    <div className="bg-white dark:bg-[#0F0F0F]">
        ...
        <div className="justify-between flex flex-row w-full">
          <h1 className="text-lg flex flex-row justify-between text-start text-zinc-500 font-semibold tracking-wide">
            <span className="flex flex-row gap-1 text-3xl text-black dark:text-white font-bold">
              <p className="text-blue-500">Latest</p>Reports
            </span>
          </h1>
      ...
      {showBanner && (
        <div className="px-5">
          <BannerResearch post={latestReport} />
        </div>
      )}
